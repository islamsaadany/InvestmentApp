...
export default function TopMovers({ investments }: Props) {
  const withPL = investments.filter((i) => i.profit_loss_pct != null);
  const sorted = [...withPL].sort((a, b) => (b.profit_loss_pct || 0) - (a.profit_loss_pct || 0));
        ...
        <div>
          ...
          {gainers.map((inv) => (
            <div key={inv.id} className="flex items-center justify-between py-1.5">
              <div>
                <span className="text-sm font-medium text-gray-900">{inv.symbol}</span>
                <span className="text-xs text-gray-400 ml-1">
                  {ASSET_TYPE_LABELS[inv.asset_type]}
                </span>
              </div>
              <span className="text-sm font-medium text-green-600">
                {formatPercent(inv.profit_loss_pct)}
              </span>
            </div>
          ))}
        </div>
