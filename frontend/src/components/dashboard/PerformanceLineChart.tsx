...
export default function PerformanceLineChart() {
  ...
  const { data: snapshots, isLoading } = usePerformance(period);
  const { currency } = useCurrency();

  const chartData = (snapshots || []).map((s) => ({
    date: new Date(s.snapshot_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
    value: currency === 'EGP' ? s.total_value_egp : s.total_value_usd,
  }));
      ...
      {isLoading ? (
        ...
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={chartData}>
            ...
            <Tooltip
              formatter={(value) => formatCurrency(Number(value), currency === 'EGP' ? 'EGP' : 'USD')}
