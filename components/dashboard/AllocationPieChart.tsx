...
export default function AllocationPieChart({
  ...
  useEffect(() => {
    fetch("/api/analysis/assets")
      .then((r) => r.json())
      .then((assets: Array<{ assetType: string; storeSymbol: string }>) => {
  ...
  // Asset types user actually owns (passed to modal as tabs)
  const ownedAssetTypes = summaries.map((s) => ({
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-6">
            ...
            <PieChart>
              <Pie
                ...
                onClick={(_, idx) => {
                  ...
                  if (item) setSelectedAssetType(item.assetType);
                }}
            ...
            <tbody>
              {sorted.map((item, idx) => (
                <tr
                  ...
                  onClick={() => setSelectedAssetType(item.assetType)}
                >
      ...
      {selectedAssetType && (
        <AssetDetailModal
          ...
          ownedAssetTypes={ownedAssetTypes}
