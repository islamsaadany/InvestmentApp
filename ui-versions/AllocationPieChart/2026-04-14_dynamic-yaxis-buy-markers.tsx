...
/**
 * Compute dynamic Y-axis domain with 10% padding above and below the data.
 */
...
function AssetMiniChartPopupWrapper({
  ...
  // Purchase date markers for My Value tab
  ...
  // Dynamic Y-axis
  const priceDomain = computeYDomain(priceChartData.map((d: any) => d.price));
  const valueDomain = computeYDomain(valueChartData.map((d: any) => d.value));
            ...
            priceChartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={priceChartData}>
                  ...
                  <YAxis tick={{ fontSize: 10 }} stroke="#94a3b8" width={60} domain={priceDomain} />
                  <Tooltip
            ...
            <>
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={valueChartData}>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  <XAxis dataKey="dateLabel" tick={{ fontSize: 10 }} stroke="#94a3b8" interval="preserveStartEnd" />
                  <YAxis tick={{ fontSize: 10 }} stroke="#94a3b8" width={60} domain={valueDomain} />
                  <Tooltip
                  ...
                  {purchaseMarkers.map((pm, i) => {
                    const matchIdx = valueChartData.findIndex((v) => v.dateLabel === pm.dateLabel);
                    if (matchIdx === -1) {
                      // Find closest
                      let closest = 0;
                      let minDiff = Infinity;
                      for (let j = 0; j < valueChartData.length; j++) {
                        const diff = Math.abs(new Date(valueChartData[j].rawDate).getTime() - new Date(pm.rawDate).getTime());
                        if (diff < minDiff) { minDiff = diff; closest = j; }
                      }
                      if (valueChartData[closest]) {
                        return (
                          <ReferenceDot key={`buy-${i}`} x={valueChartData[closest].dateLabel} y={valueChartData[closest].value} r={5} fill="#ef4444" stroke="#fff" strokeWidth={2} />
                        );
                      }
                      return null;
                    }
                    return (
                      <ReferenceDot key={`buy-${i}`} x={pm.dateLabel} y={valueChartData[matchIdx].value} r={5} fill="#ef4444" stroke="#fff" strokeWidth={2} />
                    );
                  })}
                </AreaChart>
              </ResponsiveContainer>
