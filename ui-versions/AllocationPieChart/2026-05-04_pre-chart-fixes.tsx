...
function AssetMiniChartPopupWrapper({
  ...
  // Weighted average purchase price — converted to same unit as chart (USD per unit)
  // Chart shows USD prices from AssetPriceHistory (per oz for metals, per unit for others)
  const avgPurchasePrice = (() => {
    const invs = data?.investments || [];
    if (invs.length === 0) return null;

    // Estimate EGP rate from investment data or use 53
    const egpRate = 53.13; // approximate — close enough for reference line
    ...
    for (const inv of invs) {
      ...
      // Convert to USD if purchased in EGP
      if (inv.purchaseCurrency === "EGP") {
        priceUsdPerUnit = inv.purchasePrice / egpRate;
      }

      // For gold/silver bought in grams, convert to per-ounce to match chart
      if (
        (inv.assetType === "gold" || inv.assetType === "silver") &&
        inv.weightUnit === "grams"
      ) {
        priceUsdPerUnit = priceUsdPerUnit * 31.1035; // grams → per oz
      }

      totalWeightedPrice += priceUsdPerUnit * inv.quantity;
      totalQty += inv.quantity;
    }

    return totalQty > 0 ? Math.round((totalWeightedPrice / totalQty) * 100) / 100 : null;
  })();

  // Latest/current price (last data point in the price chart)
  const latestPrice = priceChartData.length > 0
  ...
  if (latestPrice != null) priceValues.push(latestPrice);
