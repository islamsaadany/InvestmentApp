...
export default function AnalysisPage() {
  ...
  const { data: investments } = useQuery<InvestmentEntry[]>({
    ...
    queryFn: async () => {
      ...
      return (data.investments || data).map((inv: any) => ({
        symbol: inv.symbol,
  ...
  const priceChartData = useMemo(() => {
    if (!priceHistory || priceHistory.length === 0) return { dates: [], series: {} as Record<string, { date: string; price: number }[]> };
                ...
                <ResponsiveContainer width="100%" height={350}>
                  <LineChart data={priceChartData.series as any[]}>
                    ...
                    {entryPoints.map((ep, i) => {
                      // Find the chart row index closest to the purchase date
                      const chartRows = priceChartData.series as any[];
                      const matchIdx = chartRows.findIndex(
                        (row) => row.rawDate === ep.date
                      );
                      if (matchIdx === -1) return null;

                      const chartPrice = chartRows[matchIdx]?.[ep.symbol];
                      // Use the actual market price on that date (if available), else use purchase price
